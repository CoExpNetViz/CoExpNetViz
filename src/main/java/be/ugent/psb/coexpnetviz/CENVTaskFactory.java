 ...
 */
public class CENVTaskFactory extends AbstractTaskFactory implements Observer {
	...
	public CENVTaskFactory(CENVContext context) {
		...
		step = 0;  // TODO the task factory can be used only once with current impl
	}

	@Override
	public TaskIterator createTaskIterator() {
		// TODO broken: just debugging the task tunables
		return new TaskIterator(new RunJobTask(context));
		//taskIterator = new TaskIterator(new NotificationTask(this));
		//return taskIterator;
	}
	...
	public void update(Observable o, Object notificationTask) {
		 ...
		 * previous tasks in the iterator. Sometimes we need to wait for them to
		 * complete, for that we use a NotificationTask that calls this method once it's
		 * run, after which we can add the tasks of the next step to the still running
		 * task iterator.
		 */
		switch (step++) {
		case 0:
			// Run job on server, download response
			taskIterator.append(new RunJobTask(context));
			break;
		...
		case 2:
			...
			nodeTableReader = addReadTableTask(getExtractedFile("network.node.attr"), CyNode.class); // TODO these ask
																										// the user for
																										// info, it
																										// shouldn't, we
																										// can supply
		...
		case 4:
			...
			// Apply network style
			context.getVisualMappingManager().setVisualStyle(
					getStyle(CENVContext.APP_NAME, getExtractedFile("coexpnetviz_style.xml")), networkView);
	 ...
	 * 
	 * @param stylePath Path to vizmap file to get, it must contain a style with
	 *                  name equals `name`
	 */
	private VisualStyle getStyle(String name, Path stylePath) {
		// Add the visual style no sooner than when it's needed and add it only once, To
		// prevent cluttering the Style menu.
	...
	private CyTableReader addReadTableTask(Path tablePath, Class<? extends CyIdentifiable> type) {
		CyTableReader tableReader = context.getCyTableReaderManager().getReader(tablePath.toUri(),
	...
	private void addApplyTableTask(CyTableReader tableReader, Class<? extends CyIdentifiable> type, CyNetwork network) {
		...
		TaskIterator tasks = context.getImportDataTableTaskFactory().createTaskIterator(table, false, false,
				Collections.singletonList(network), rootNetwork, joinColumn, type);
		taskIterator.append(tasks);
	}

	private CyNetworkReader addReadNetworkTask() {
		CyNetworkReader networkReader = context.getCyNetworkReaderManager()
				.getReader(getExtractedFile("network.sif").toUri(), null);
