    ...
    private void executeJobOnServer(HttpEntity entity, Path resultPath) throws IOException, JobServerException {
        ...
        try {
            ...
            if ("application/x-gtar".equals(contentType)) {
            	...
            	String message = (String)errorNode.get("message");
            	throw new JobServerException(message.replace("<pre>", "").replace("</pre>", "").replace("<br/>", "\n").replace("&gt;", ">").replace("&lt;", "<").replace("&quot;", "\"")); // Just show the message generated by the server
            }
            else {
